...
export function aStarAlgorithm(grid, animations, finishNode, endNode) {
  let visitedOrder = aStar(grid, animations, finishNode, endNode);
  let nodesInShortestPathOrder = findShortestPath(endNode);
  return [visitedOrder, nodesInShortestPathOrder];
}
// A* Search algorithm, finds the shortest path from finishNode to endNode
function aStar(grid, animations, finishNode, endNode) {
  // Initialize priority queue
  let openSet = new PriorityQueue();
  // Set initial values of A* search variables for finishNode, enqueue afterwards
  finishNode.gcost = 0;
  finishNode.hcost = calculateHeuristicValue(finishNode, endNode);
  finishNode.fcost = finishNode.hcost;
  openSet.enqueue(finishNode);
  // Initialize an array tracking the nodes visited order
  let visitedOrder = [];
  // While the openSet set is not empty, do...
  while (!openSet.isEmpty()) {
      ...
      for(let neighbor of neighbors) {
            ...
            if(tempG < neighbor.gcost) {
                  neighbor.previousNode = current;
                  neighbor.gcost = tempG;
                  neighbor.hcost = calculateHeuristicValue(neighbor, endNode);
                  neighbor.fcost = neighbor.gcost + neighbor.hcost;
                  if(!openSet.find(neighbor)) {
                        openSet.enqueue(neighbor);
                  }
...
function calculateHeuristicValue(node, endNode) {
  // Manhattan distance
